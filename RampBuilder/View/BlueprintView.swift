...
struct BlueprintView: View {
    ...
    var jumpColor = Color.blue
    var trajectoryColor = Color.green
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ForEach(0..<Int(geometry.size.height / self.calcStep(geometry.size)), id: \.self) { line in
                Group {
                    ...
                    if line >= 0 {
                        ...
                        .offset(x: -15, y: self.yOffset(Double(line), size: geometry.size) - 5)
                    }
                }
            } //foreach
            ...
            Path { path in
                ...
                let takeoffEndPoint = CGPoint(x: step * CGFloat(self.params.takeoffLength), y: geometry.size.height)
                let takeoff0AnglePoint1 = CGPoint(x: 0, y: geometry.size.height - CGFloat(self.params.takeoff.height) * step)
                let takeoff0AnglePoint2 = CGPoint(x: CGFloat(self.params.takeoffLength) * step, y: geometry.size.height - CGFloat(self.params.takeoff.height) * step)
                ...
                let landingStartPoint = CGPoint(x: step * CGFloat(self.params.takeoffLength) + (CGFloat(self.params.gap) * step) - (CGFloat(self.params.landing.table) * step), y: geometry.size.height - (CGFloat(self.params.landing.height) * step))
                let tablePoint = CGPoint(x: step * CGFloat(self.params.takeoffLength) + (CGFloat(self.params.gap) * step), y: geometry.size.height - (CGFloat(self.params.landing.height) * step))
                let landingPoint1 = CGPoint(x: step * CGFloat(self.params.takeoffLength) + (CGFloat(self.params.gap) * step), y:
                geometry.size.height - (CGFloat(self.params.landing.height) * step))
                let landingPoint2 = CGPoint(x: step * CGFloat(self.params.takeoffLength) + (CGFloat(self.params.gap) * step) + (CGFloat(self.params.landingLength) * step), y: geometry.size.height)
                let landing0AnglePoint = CGPoint(x: step * CGFloat(self.params.takeoffLength) + (CGFloat(self.params.gap) * step) + (CGFloat(self.params.landingLength) * step), y: geometry.size.height - (CGFloat(self.params.landing.height) * step))
                
                // Takeoff
                if self.params.takeoff.angle == 0 {
                    path.move(to: takeoff0AnglePoint1)
                    path.addLine(to: takeoff0AnglePoint2)
                } else {
                ...
                if self.params.landing.angle == 0 {
                    path.addLine(to: landing0AnglePoint)
                }
                path.addLine(to: landingPoint2)
            }
            .stroke(self.jumpColor, lineWidth: 4)
            ...
            .stroke(self.trajectoryColor, style: StrokeStyle(lineWidth: 2, dash: [10]))
        }
    }
}
...
//        return Int(max(maxHeight, maxlength)) + 1
        ...
        let result = Int(max(maxHeight, maxlength)) + 1 > 20 ? 20 : Int(max(maxHeight, maxlength)) + 1
